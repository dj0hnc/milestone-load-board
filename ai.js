...
const SYSTEM = [
  ...
  'TO FIND/LOOK UP A TRUCK by number/owner/driver, ALWAYS use search_trucks — it searches the FULL roster',
  'including SUBHAULERS that have no Samsara GPS (e.g. BT02 = Butler Trucking). samsara_locations and',
  'truck_status are GPS-only, so a subhauler will look "missing" there even though it exists. If a truck',
  'seems missing, search_trucks before saying it does not exist.',
  'ACTION tools (flag a truck shop/off + log, run auto-plan, ASSIGN a truck to an order, UNASSIGN/remove a',
  'truck from an order, finalize+push to NewMile) only',
